---
name: imops
description: Load when the user asks to run the new IntentMill operations flow, initialise an issue worktree, initialise t2p ticket artifacts, create im-draft.md, run the draft grill loop, finalise im-spec.md/im-plan.md, develop and unit test from IntentMill artifacts, or invokes imops cap1/cap2/cap3/cap4/cap5/cap6/cap7. Use for IntentMill-managed project repos from ssot-config.json.
---
...
- cap4 `draft-grill-loop` is itself a loop and uses its own completion check instead of a sibling gate.
...
Read `references/capability-3-create-draft/create-draft.md`, produce `im-draft.md`, then run `references/gate-3-create-draft/gate.md`. Cap4 owns creation and maintenance of `im-grill.md`.
...
Purpose: use `im-draft.md` to interview the user until blocking product/architecture decisions are resolved. Each round updates `ticket-worktree-t2p refs path/im-grill.md` and `im-draft.md`.
...
Read `references/capability-5-finalize-spec-plan/finalize-spec-plan.md`, produce both artifacts, then run `references/gate-5-finalize-spec-plan/gate.md`. Cap6 may start only after gate5 passes.
...
Purpose: implement from `im-spec.md` and `im-plan.md`, create or complete ticket-scoped unit tests under `ticket-worktree-t2p tests path`, run the relevant unit tests, and stop for cap4 when new human decisions are discovered.

Read `references/capability-6-dev-unit-test/dev-unit-test.md`, perform development and tests, then run `references/gate-6-dev-unit-test/gate.md`.
