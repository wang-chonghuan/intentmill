...
3. Enter the issue worktree, normally `.workspace/<project-key>--<ISSUE-ID>`.
4. Require the target project to provide a local t2p skill at `.agents/skills/t2p/SKILL.md`. If it is missing, stop with an error:
   ...
   ```
5. Check whether `.t2p/tickets/<ISSUE-ID>/` exists in the issue worktree.
...
7. If the directory already exists, call the project t2p requirement refresh capability:
   ...
   t2p-req <ISSUE-ID>
   ```
8. After the t2p action finishes, report the ticket directory path and which action was used.
...
- ticket artifact directory: `.t2p/tickets/<ISSUE-ID>/`

## Rules

- Do not run t2p from the IntentMill repository root.
- Do not run t2p from the base `.workspace/<project-key>` clone.
- Do not create `.t2p/tickets/<ISSUE-ID>/` manually when the t2p skill is available; use `t2p-new`.
